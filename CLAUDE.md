...
| `SANDBOX_DRIVER` | no | `docker` | Selects the sandbox driver. `docker` starts sandbox containers on the local daemon; `kubernetes` schedules a pod through the cluster API instead. An unrecognised value fails startup naming it |
