...
- **Trajectory paths must be workspace-relative before they reach the drift engine.** An agent CLI reports absolute paths inside the sandbox (`/workspace/...`); `contract.MatchPath` compares relative patterns and rejects an absolute candidate outright, on purpose. `runner/session.go` calls `trajectory.Relativize` on every session's and probe's events, stripping `Executor.WorkDir()`. Skipping that does not merely drop a signal — it breaks the drift score in *both* directions at once: step coverage, checkpoints and focus fall to zero because nothing matched, while violation and order rise to a vacuous 1.0 because nothing could be violated or mis-ordered either. The result is a constant (100×(0.15+0.25) = 40.0 at `DefaultWeights`) that reads exactly like a measurement, and it is below every grade threshold. A real report scored ten tasks that way. `drift.Result.Unmeasurable` and `UnmeasurableThreshold` exist so it cannot happen silently again.
- **`InstallSkill` installs shipped content only.** The directory it is handed is the authoring skill dir, which also holds the eval sidecar — including every task's `oracle/solve.sh`. `claudecode.copyTree` filters with `lint.Excluded`; without it the reference solution lands in the workspace of the agent being measured, which a real report confirmed. `runner.stageRunWorkspace` already excludes `oracle/` for exactly this reason, and an unfiltered install put it straight back.
- **The published score is `Report.Headline`, a 0–100 minimum of member Effectiveness, and it is the only composite.** There used to be a second — a letter grade derived from drift adherence, rendered in a bare "Grade" column beside Effectiveness. It is gone: `MemberReport.DriftGrade` is retained as an always-empty `omitempty` field so existing decoders keep working. The headline's bootstrapped 95% CI is gone too; it bootstrapped the *mean* while the headline is the *minimum*, and at a two-member panel it could only ever reproduce `[min, max]`.
