...
- **`eval_suites.origin` means "nobody reviewed this", and a client can only ever declare the weaker half.** `whetstone suite push` compares the suite's ref against the ref the generator recorded. It sends `unreviewed: true` when the two match, and the server routes that to `PutDerived`. No field claims authored, because a pusher that claims it clears its own scan hold. Only the review route raises a suite, through `Registry.MarkAuthored`, behind an authenticated user. Two consequences look like bugs and are not. An older `whetstone` sends no field and still pushes as authored, which is the price of `omitempty` on a request-body field. One edited byte also stops the declaration, because the rule assumes accident rather than attack.
...
| `CLAUDE_CODE_OAUTH_TOKEN` | no | — | Bills the *panel* to a Claude subscription (`claude setup-token`). The judge still needs one of the two credentials above |
