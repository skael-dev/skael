...
| `SANDBOX_K8S_IMAGE` | with `kubernetes` | — | Image a session pod runs. The Kubernetes driver resolves a published image; it does not build one, so this must already be in a registry. Point it at `ghcr.io/skael-dev/whetstone-base:<n>`, the image the release workflow publishes. `Options.Validate` refuses to start without it, so a missing value fails fast with a clear message instead of silently pulling a public image |
