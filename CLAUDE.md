...
| `SANDBOX_K8S_IMAGE` | no | `ghcr.io/skael-dev/whetstone-base:<n>` | Image a session pod runs. The Kubernetes driver resolves a published image; it does not build one, so this must already be in a registry |
