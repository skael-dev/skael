...
**`cmd/skael-worker`** — Eval queue worker. Polls `POST /api/eval/jobs/claim`, materialises a local whetstone workspace from the claimed job's skill bundle and suite, runs the evaluation against a sandbox selected by `SANDBOX_DRIVER` (`docker`, the default, or `kubernetes`), heartbeats the lease while running, and posts the report back. The LLM judge always runs through the direct Anthropic API gateway (`ANTHROPIC_API_KEY`), never a subscription CLI on PATH. Panel execution is not the same guarantee: the claude-code agent adapter declares `AuthDirs: ["~/.claude", "~/.config/claude"]`, which `internal/eval/runner/session.go` mounts into every sandbox, so a panel member authenticates with whatever host credentials it finds there — subscription-backed wherever those directories exist on the worker's host. One poll loop, one job at a time per process; run more replicas for throughput.
...
- **Comment density: `just check` runs `checks/comment-density.sh`, which fails a file whose diff against `main` is more than 25% comment.** Aim at ~10%, which is where `internal/skill` and `internal/platform` sit; the check is a backstop, not the target. Comment the decision a reader cannot infer: an import cycle, a security constraint, a "do not 'fix' this", a silent failure mode. State a rationale once, nearest the code that depends on it. This was prose for a long time and was ignored anyway, which is why it is now a script.
...
- **The tap's formula-to-cask migration is a two-step release, and the second step is manual.** `.goreleaser.yaml` publishes `homebrew_casks`; `tap_migrations.json` in `skael-dev/homebrew-skael` and the deletion of the old `Formula/*.rb` must wait until the first cask release has actually published, or `brew update` migrates users to a cask that does not exist yet. Casks are macOS-only, so every install doc offers `install.sh` for Linux rather than a `brew install` that fails.
- **Homebrew casks are generated from `.goreleaser.yaml`, not edited in the tap.** `skael-dev/homebrew-skael` is a publish target; its `.rb` files say "DO NOT EDIT". One `homebrew_casks:` entry per binary — `skael` and `whetstone` — rather than one archive carrying several, because `install.sh` derives its download name from a single `BINARY` and the docs describe each archive as holding one binary. `license` is accepted there and renders nothing: a cask has no license stanza.
- **`InstallSkill` installs shipped content only**, filtered by `lint.Excluded`. The directory it is handed is the authoring skill dir, which holds the whole eval sidecar; unfiltered, the eval set lands in the workspace of the agent being measured.
- **`Report.Headline` (0–100, minimum member Effectiveness) is the only composite** and the only published score. A drift letter grade and a bootstrapped CI were removed; `quality.RecordOutput.DriftGrade` remains as an always-empty `omitempty` field so existing decoders keep working.
...
- **The judge retries, and what reaches it is bounded.** `cmd/skael-worker` sets `MaxRetries: 3` (`judgeGatewayOptions`), the value `whetstone` already used — left at zero, one 429 on any of about 30 grade calls threw away a whole panel run. `internal/eval/llm/api` reads `retry-after`, clamps it to 120s, jitters by plus or minus 20%, and returns at once on a cancelled context. A grade that still fails is dropped from the denominator and named in `Report.DroppedGrades`; only a run with *no* grade at all is an error. The transcript sent to the judge is capped at 256 KB (a 192 KB head plus a 64 KB tail, marker between), and the rendered outputs block at 128 KB — an unbounded transcript overflowed the context for a 400, which is not retryable, so no retry policy helps.
- **A baseline is reused, not re-run.** A baseline installs no skill, so `store.ReusableBaseline` matches on the suite ref, the task, the agent, the model and the recorded `agent_version`, with a status of ok and an age under 30 days; `store.CopyBaseline` copies the run *and its grade* into the current eval. A copied run with no grade would count as a session that passed nothing. `--fresh-baseline` turns it off.
- **`--concurrency` and `--grade-concurrency` bound different resources.** A sandbox container is bounded by CPU and memory, a judge call by the account's rate limit. Each falls back to its own default (6 sessions, 8 judge calls), never to the other's. The worker reads `WORKER_CONCURRENCY` and `WORKER_GRADE_CONCURRENCY`.
- **`runner.Options.OnComplete` fires once per finished session, from that session's own goroutine**, which is what lets `whetstone` grade a session while the rest still run. It is never called for a probe. Probes queue against the same semaphore as the runs, so `Execute` has one `wg.Wait()`.
- **The quota gate holds starts, never a session in flight.** `internal/eval/runner/quota.go` is a small type with an injected clock: above 0.95 reported utilization it holds new session starts for as long as the throttled session's own backoff. That is what makes the default concurrency 6 rather than 4.
- **`llm.ClassFast` is requested by `gen.outline` only** — every judge call asks for `ClassStrong`. A single-entry `LLM_MODEL` therefore serves both classes with no consequence; the second entry's other live effect is the panel's floor member at the deep tier.
- **A `rate_limit_event` is telemetry, and its allowed statuses are a family.** `internal/eval/agent/parse.go` matches `allowed` as a *prefix*, because a subscription past 75% of its window emits `allowed_warning` on a session that succeeds. This line has produced a false throttle twice: first by flagging the event's mere presence, then by accepting only the literal `"allowed"`. Both times the symptom was identical and misleading — every session retried to death, then `score.MemberScore: no scored evals`, which reads like a broken worker rather than a parser that threw away good runs. Only the subscription path emits these events at all, so the split mode is where it surfaces. `Meta.RateLimitUtilization` carries the number so `warnOnQuota` can say "94% of your seven_day window" once per run, before anything actually fails.
...
- **`SANDBOX_DRIVER=kubernetes` refuses any run that is not `NetFull` until `SANDBOX_K8S_NETWORK_POLICY=true`.** A CNI that does not enforce NetworkPolicy accepts the policy object and ignores it, so an unasserted cluster would run every "restricted" session unrestricted while every test still passed. `whetstone doctor --check-egress` settles it against the real cluster.
- **The Kubernetes driver cannot install `spec.DepsDecl` dependencies, because it resolves a published image rather than building one.** A declared dependency is refused by name. Do not "fix" this into a run on the base image: the failures would be recorded as the skill's fault.
- **`SANDBOX_DRIVER=northflank` cannot scope egress to one session, and cannot restrict it at all on Northflank Cloud.** Northflank's network policies are documented for a BYOC cluster only, are configured through its web UI with no API or CLI surface, and are scoped to a project and its workloads rather than to one sandbox. The driver therefore checks each run's requested domains against `SANDBOX_NF_ALLOWED_DOMAINS`, which the operator sets to whatever their own cluster enforces, and refuses a skill asking for anything outside it. On Northflank Cloud no egress restriction is documented, so every restricted run is refused and only `NetFull` works — which means a release cannot be scored on Northflank Cloud. Do not "fix" this by dropping the check: the alternative is a run that reports a restricted network and has none.

- **A contest's verdict is computed on the server, never sent by a worker.** `internal/contest/routes.go` reduces every candidate's report and calls `contest.Decide`; the worker posts measurements only. A worker that could post a verdict could forge one, and a verdict computed in two places drifts from the rule.
- **`contest.Decide` drops tied tasks before the sign test.** A suite both candidates pass carries no evidence, and counting those tasks as agreement lets a long easy suite manufacture significance. The consequence surprises everyone: 7-2 is p=0.18, and below `MinDecidedTasks()` (six) no split can beat chance at all, which is why the detail line blames the suite rather than the skills.
- **`propose.Prompt` must never receive a task prompt, a suite or a workspace.** The missed expectations and the grader's evidence are the whole input, and that is the entire overfitting defence — there is no held-out split, because one proposal cannot compound a fit and splitting twelve tasks leaves six to validate on. `TestPrompt_CarriesTheExpectationsAndNoTask` guards it.
- **A proposal may only rewrite prose the bundle already has.** `keepKnownFiles` drops any path the bundle lacks, and only `.md`/`.txt` are read in the first place. A generator that can name a new path can write a script into a bundle an agent runs in a sandbox.
- **`skill.Pack` normalises every tar header** — zero mtime, no uid/gid, USTAR, executable bit only. `tar.FileInfoHeader` carries the packing clock, which made identical content hash differently and broke the unchanged-republish short-circuit, storage dedupe, and one publish test that only failed when CI straddled a second.
- **`testutil.SetupTestDB` shares one Postgres per test binary** and hands each test a database made with `CREATE DATABASE ... TEMPLATE`. A container per test cost `just check` about half an hour. The container is left to the testcontainers reaper on purpose: a `t.Cleanup` tears it down after the first test.
...
| `WORKER_CONCURRENCY` | no | `1` | Concurrent sandbox sessions. Must be a positive integer |
| `WORKER_GRADE_CONCURRENCY` | no | whetstone's default (8) | Concurrent judge calls. A separate resource from the sessions: a container is bounded by CPU and memory, a judge call by the account's rate limit. Must be a positive integer |
...
| `SANDBOX_DRIVER` | no | `docker` | Selects the sandbox driver. `docker` starts sandbox containers on the local daemon; `kubernetes` schedules a pod through the cluster API instead; `northflank` runs each session as a service in a Northflank project. An unrecognised value fails startup naming it |
| `SANDBOX_K8S_NAMESPACE` | with `kubernetes` | — | Namespace the driver creates session pods in. This namespace must hold nothing but session pods: `pods/exec` is the driver's entire privilege surface, and whoever holds it can execute in any pod there |
| `SANDBOX_K8S_IMAGE` | no | `imagespec.PublishedBaseImage` (`ghcr.io/skael-dev/whetstone-base:<n>`) | Image a session pod runs. The driver resolves a published image and does not build one, so this must already be in a registry. The default is the image the release publishes, so an operator who wants the shipped environment does not name it. Set it to run your own base |
...
| `SANDBOX_K8S_HARDWARE_ISOLATED` | no | unset | Asserts the runtime is hardware-isolated. Accepted only as the exact lowercase string `true`; any other value, including `TRUE` or `1`, reads as unasserted and the driver refuses runs that need isolation, same as the docker driver with none configured |
...
| `SANDBOX_NF_ALLOWED_DOMAINS` | for restricted runs | — | Comma-separated domains the operator's own project egress policy permits. Spaces around a comma are trimmed |
| `SANDBOX_NF_NETWORK_POLICY` | no | unset | Asserts that the domains above are actually enforced by the project's egress policy. Accepted only as the exact lowercase string `true`; any other value reads as unasserted and the driver refuses a restricted run |
| `SANDBOX_NF_HARDWARE_ISOLATED` | no | unset | Asserts the plan is hardware-isolated. Accepted only as the exact lowercase string `true`; any other value reads as unasserted and the driver refuses runs that need isolation, same as the docker driver with none configured |
...
Supported, and how `docker compose --profile worker up` runs it (`Dockerfile.worker`). This section covers the **Docker driver** (`SANDBOX_DRIVER=docker`, the default). The worker starts sandbox containers as *siblings* through a mounted `/var/run/docker.sock` — not Docker-in-Docker. Two constraints follow, both load-bearing, and **both apply to the Docker driver only** — see below for why the Kubernetes driver needs neither:

- **`WORKER_RUN_ROOT` must be bind-mounted at the same path on both sides** (`-v /var/lib/skael/run:/var/lib/skael/run`). Session workspaces are bind-mounted into each sandbox and the *host* daemon resolves that path. A container-local path names nothing on the host, and Docker creates a missing bind source as an empty directory rather than failing — so the sandbox starts with no task and no skill and scores as a skill that did nothing. `requireHostSharedRoots` in `cmd/skael-worker` refuses to start containerized without it, because nothing downstream can distinguish that from a genuinely bad skill.
...
### Running the worker against a Kubernetes cluster

`SANDBOX_DRIVER=kubernetes` schedules a session as a pod through the cluster API instead of talking to a Docker socket. This is the main practical reason the driver exists: a worker on a containerd node, or a worker that must not hold a Docker socket at all, has neither host-path constraint above to satisfy. `WORKER_RUN_ROOT` does not apply, because nothing is bind-mounted, and no node needs a Docker socket. Grant the worker's service account only `deploy/kubernetes/worker-rbac.yaml`, and put nothing but session pods in that namespace — `pods/exec` is the driver's entire privilege surface, and whoever holds it can execute in any pod there.

- **The Kubernetes driver resolves a published image; it does not build one.** `SANDBOX_K8S_IMAGE` names an image already in a registry, and defaults to `imagespec.PublishedBaseImage`, which `resolve.FromEnv` supplies. `Options.Validate` still requires a non-empty value, so a driver constructed directly rather than through `resolve` fails fast instead of pulling nothing. The default is `ghcr.io/skael-dev/whetstone-base:<n>`, where `<n>` is the version suffix of `imagespec.DefaultBaseTag`. The release workflow's `images` job publishes it from `imagespec.BaseDockerfile(false)` (`go run ./cmd/whetstone --print-base-dockerfile`), tagged by that suffix, never by the release version — the constant is bumped only when the base image's contents change, and tagging per release would publish a new environment every time and split every score trend. Bumping `DefaultBaseTag` requires a release before a Kubernetes worker can pull the new base; until then it keeps resolving the old tag.
