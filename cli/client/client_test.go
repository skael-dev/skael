...
func TestUploadEvalSuite_SendsExpectedBody(t *testing.T) {
	...
	resp, err := c.UploadEvalSuite(EvalSuiteUploadRequest{Skill: "deploy-helper", SpecVersion: 3, Checks: checks, Archive: []byte("archive-bytes")})
	if err != nil {
...
func TestUploadEvalSuite_RespectsClientTimeout(t *testing.T) {
	...
	start := time.Now()
	_, err := c.UploadEvalSuite(EvalSuiteUploadRequest{Skill: "deploy-helper", SpecVersion: 1, Checks: []EvalSuiteCheck{{TaskID: "t00", OK: true}}, Archive: []byte("x")})
