...
`
	...
	_, created, err := importSingleSkill(ctx, rootDir, ds, src, skillStore, importStore, storage, nil)
	require.NoError(t, err)
...
`
	...
	_, _, err = importSingleSkill(ctx, rootDir, ds, src, skillStore, importStore, storage, nil)
	require.NoError(t, err)
